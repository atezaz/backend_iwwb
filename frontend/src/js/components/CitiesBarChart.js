...
function CitiesBarChart() {
    ...
    const ammountOfShownCties = 10; // is the number of cities that will be shown in the chart

    // properties of the chart size
    const margin = {top: 20, right: 50, bottom: 80, left: 35},
    ...
    function drawHeatMap(dataset) {
        ...
        const y = d3.scaleLinear()
            .domain([0, dataset[0][1] + dataset[0][1] * 0.05])
            .range([ height, 0]);
...
const domNode = document.getElementById('cities-bar-chart');
const root = createRoot(domNode);
root.render(<CitiesBarChart />);
